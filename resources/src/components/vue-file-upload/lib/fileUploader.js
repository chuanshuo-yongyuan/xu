...
class FileUploader {
  ...
  addToQueue (files) {
    let uploader = this
    let filess = uploader._isFileList(files) ? _.toArray(files) : [files]

    //  file可能是File,htmlInputElement,Object
    function _uploadFile (_file) {
      var fileAlias = new FileAlias(_file)
      if (uploader._isValidFile(fileAlias, uploader.filters)) {
        var fileItem = new FileItem(uploader, _file, fileAlias)
        uploader.queue.push(fileItem)
        uploader.onAddFileSuccess(fileItem)
      } else {
        uploader.onAddFileFail(_file, uploader.filters[uploader.failFilterIndex])
      }
    ...
    filess.forEach((file) => {
      if (('|jpg|png|jpeg|bmp|gif|webp|'.indexOf(file.type) !== -1) || file.type.slice(0, file.type.lastIndexOf('/')) !== 'image') {
        _uploadFile(file)
      } else {
        var compress = new Compress()
        compress.photoCompress(file, function (file) {
          _uploadFile(file)
        })
      }
    })
  ...
  uploadItem (fileItems) {
    ...
    var fileItem = fileItems || this.getNotUploadedItems()[0]
    if (fileItem) {
